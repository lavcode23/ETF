...
def _fig_equity(runner):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=eq["Date"], y=eq["Portfolio_Value"], name="Strategy", line=dict(color="#2E86AB", width=2.5)))
    fig.add_trace(go.Scatter(x=b.index, y=b_norm, name="NIFTYBEES", line=dict(color="#A23B72", width=2, dash="dash")))
    fig.update_layout(title="Equity Curve", xaxis_title="Date", yaxis_title="Value", hovermode="x unified", height=420, template="plotly_white")
    return fig


def _fig_drawdown(runner):
    eq = runner.portfolio.get_equity()
    eq["Date"] = pd.to_datetime(eq["Date"])
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dd.index, y=dd, fill="tozeroy", name="Drawdown", line=dict(color="#E63946")))
    fig.update_layout(title="Drawdown", xaxis_title="Date", yaxis_title="DD %", height=300, template="plotly_white")
    return fig


def _fig_monthly_bars(runner):
    mr = runner.portfolio.monthly_returns().dropna()
    colors = ["#06D6A0" if v > 0 else "#EF476F" for v in mr["Monthly_Return"]]
    fig = go.Figure(data=go.Bar(x=pd.to_datetime(mr["Date"]), y=mr["Monthly_Return"], marker_color=colors))
    fig.update_layout(title="Monthly Returns", xaxis_title="Date", yaxis_title="Return %", height=320, template="plotly_white")
    return fig


def _fig_sector_rotation(runner):
    ...
    fig = go.Figure(data=go.Heatmap(z=pivot.values, x=pivot.columns.tolist(), y=pivot.index.tolist(), colorscale="YlOrRd", colorbar=dict(title="Alloc %")))
    fig.update_layout(title="Sector Rotation", xaxis_title="Month", yaxis_title="ETF", height=400, template="plotly_white", xaxis=dict(tickangle=45))
    return fig
...
def main():
    ...
    with st.sidebar:
        ...
        capital = st.number_input("Initial Capital (INR)", value=1000000, step=100000, min_value=100000)
    ...
    runner = _run(start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"), top_n, capital)
    ...
    tab_over, tab_perf, tab_alloc, tab_trades, tab_dl = st.tabs(["Overview", "Performance", "Allocation", "Trades", "Download"])

    with tab_over:
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total Return", f"{p['Total_Return_%']} %", delta=f"vs Bench {p['Benchmark_Total_Return_%']} %")
        c2.metric("CAGR", f"{p['CAGR_%']} %", delta=f"Alpha = {p['Alpha_%']} %")
        ...
        with col_risk:
            ...
            for label, key in [("Annual Vol", "Annual_Volatility_%"), ("Sortino", "Sortino_Ratio"), ("Calmar", "Calmar_Ratio"), ("Monthly Win %", "Monthly_Win_Rate_%"), ("Hit Rate %", "Hit_Rate_%")]:
    ...
    with tab_perf:
        ...
        st.dataframe(pd.DataFrame(list(p.items()), columns=["Metric", "Value"]), use_container_width=True, hide_index=True)
        if not runner.importance_df.empty:
            st.subheader("Feature Importance")
            fig_imp = px.bar(runner.importance_df.head(12), x="Importance", y="Feature", orientation="h", color="Importance", color_continuous_scale="Blues")
    ...
    with tab_alloc:
        ...
        if cur is not None and not cur.empty:
            ...
            pie_df = pd.concat([cur[["ETF", "Weight"]], pd.DataFrame([{"ETF": "CASH", "Weight": cash_w}])], ignore_index=True)
            fig_pie = px.pie(pie_df, names="ETF", values="Weight", color_discrete_sequence=px.colors.qualitative.Set2)
            fig_pie.update_layout(height=380)
            st.plotly_chart(fig_pie, use_container_width=True)
            display = cur.copy()
            display["Weight %"] = (display["Weight"] * 100).round(2)
            st.dataframe(display[["ETF", "Weight %", "FinalScore", "ML_Probability", "TechScore"]], use_container_width=True, hide_index=True)
        else:
    ...
    with tab_dl:
        ...
        if st.button("Generate & Download Report", type="primary"):
            ...
            with open(path, "rb") as f:
                st.download_button("Download Excel Report", data=f, file_name=os.path.basename(path), mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
